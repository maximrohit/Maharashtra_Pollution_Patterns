...
Base_data_mum_no2<-Base_data_pollutant[which(Base_data_pollutant$City_poll=='MUMBAI' & 
                                               Base_data_pollutant$Type_of_Location=='Residential'
),
c('Type_of_Location','City_poll','year_number','month_number',
  'SO2_Median')]
min(Base_data_mum_no2$SO2_Median,na.rm=T)#2 
max(Base_data_mum_no2$SO2_Median,na.rm=T)#60.8
...
Base_data_mum_no2_2007<-Base_data_mum_no2[which(Base_data_mum_no2$year_number>=2010),]
nrow(Base_data_mum_no2_2007)##2007 108, 2010 72
...
# plot(Base_data_mum_no2_2007$SO2_Median)
#2010 onwards
 Train<-Base_data_mum_no2_2007[1:60,]
 Validate<-Base_data_mum_no2_2007[61:72,]
# View(Validate)
...
#2007 onwards
Train<-Base_data_mum_no2_2007[1:96,] 
Validate<-Base_data_mum_no2_2007[97:108,]
...
kpss.test(ts_train)
###2007
#KPSS Level = 1.4796, Truncation lag parameter = 3, p-value = 0.01
##null hypothersis of serie sbeing stationary can be rejected
##2010
##KPSS Level = 0.43195, Truncation lag parameter = 3, p-value = 0.06338
## null hypothersis of serie sbeing stationary can't be rejected
#########################################
adf.test(ts_train, alternative="stationary")
#2007 onwards
#Dickey-Fuller = -4.3481, Lag order = 4, p-value = 0.01
##not stationary null hupothesis can't be rejected, opposit result, adf pointing to presence of unit root
#2010 onwards
#Dickey-Fuller = -3.4185, Lag order = 3, p-value = 0.06145
## not stationary null hupothesis can be rejected, so unit root doen;t exist
...
saveRDS(autoarima, "E:/MS/Timeseries/code/autoarima_2010_mumbai_So2_onwards.rds")
...
# -0.5640  0.0788  -0.2445  -0.2023  0.4012  -0.0967
# s.e.   0.1163  0.1263   0.1310   0.1177  0.1337   0.0340
# 
# sigma^2 estimated as 5.34:  log likelihood=-212.94
# AIC=439.89   AICc=441.18   BIC=457.77
##2010
# ARIMA(0,1,1) 
# 
# Coefficients:
...
# s.e.   0.1665
# 
# sigma^2 estimated as 1.628:  log likelihood=-97.85
# AIC=199.71   AICc=199.92   BIC=203.86
...
# Test set 2.042791 3.312495 2.228363 41.53065 50.80924
# 2010
#ME    RMSE      MAE       MPE     MAPE
#Test set 0.4464387 2.37858 1.601187 -14.14503 42.52101
...
#options[36130,]
# i j k l  m  n o p q
# 36130 9 5 1 5 17 17 1 1 1
#ME     RMSE      MAE      MPE     MAPE
#Test set 0.8447278 2.246418 1.142522 10.64884 24.60781

Stl = stl(ts_train,s.window=9,t.window=5,
          l.window=1,s.jump=5,t.jump=17,
          l.jump=17, robust =TRUE,s.degree=1,t.degree=1,l.degree=1)
...
##base stl 2010 ets
#ME     RMSE      MAE      MPE     MAPE
#Test set 1.238199 2.237276 1.394325 23.11473 30.92103
##base stl 2010 arima
#ME     RMSE      MAE      MPE     MAPE
#Test set 0.8912757 2.052666 1.213444 10.52235 26.63076
...
data_120<-as.data.frame(cbind(month=1:12,SO2=var[1:12,]))

View(data_120)
g<-ggplot(data_120,aes(x=month,y=SO2, colour=SO2))+geom_line()+ 
  scale_x_continuous(breaks = seq(1, 12, by = 1))+
    labs(title="Mumbai SO2 Seasonality",
...
g
ggsave('E:/MS/Timeseries/Images/Mumbai_SO2_12.png', plot = g,width = 10, height = 7.5)
scale_x_continuous(breaks = rep(seq(3, 12, by = 3),times=5))
